...
class SMTPValidateEmail
{
    ...
    protected function connect($host)
    {
        ...
        $this->socket = @stream_socket_client($this->host, $errnum, $errstr, $this->connect_timeout,
            STREAM_CLIENT_CONNECT, stream_context_create(array()));
        // connected?
        if (!$this->connected()) {
    ...
    protected function quit()
    {
        ...
        if ($this->state['helo']) {
            $this->send('QUIT');
            $this->expect(array(self::SMTP_GENERIC_SUCCESS, self::SMTP_QUIT_SUCCESS), $this->command_timeouts['quit'],
                true);
        }
    }
    ...
    protected function send($cmd)
    {
        ...
        // did the send work?
        if ($result === false) {
            throw new SMTP_Validate_Email_Exception_Send_Failed('Send failed ' . 'on: ' . $this->host);
        }
    ...
    protected function expect($codes, $timeout = null, $empty_response_allowed = false)
    {
        ...
        try {
            ...
            if (($empty_response_allowed === false && ($code === null || !in_array($code,
                            $codes))) || $code == self::SMTP_SERVICE_UNAVAILABLE
            ) {
